        ...
        <div className="max-w-7xl mx-auto container-padding py-12">
          ...
          <div className="mb-12">
            <div className="flex flex-col md:flex-row md:items-center gap-6 mb-6">
              <div 
                className="w-20 h-20 bg-gradient-primary rounded-2xl flex items-center justify-center shrink-0 shadow-lg"
              >
          ...
          <div className="relative w-full h-[300px] md:h-[450px] rounded-2xl overflow-hidden mb-16 shadow-xl">
            <img
              src={data.imageUrl || "https://images.unsplash.com/photo-1519003722824-194d4455a60c?ixlib=rb-4.0.3&auto=format&fit=crop&w=2000&q=80"}
              alt={data.title}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12">
               <div className="max-w-3xl">
                  <h3 className="text-2xl md:text-3xl font-bold text-white mb-4 drop-shadow-md">Overview</h3>
                  <p className="text-white/90 font-medium text-lg leading-relaxed drop-shadow-sm line-clamp-3 md:line-clamp-none">{data.overview}</p>
               </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
            {data.benefits.slice(0, 3).map((benefit, index) => {
              return (
                <div key={index} className="card-glass p-8 hover:border-primary/40 hover:-translate-y-1 transition-all duration-300 shadow-sm hover:shadow-xl hover:shadow-primary/10">
                  ...
                  <div className="text-sm font-medium text-muted-foreground leading-relaxed">{benefit.description}</div>
                </div>
              );
            })}
          </div>
              ...
              <section className="mb-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {data.features.map((feature, index) => (
                    <div key={index} className="flex flex-col gap-3 p-5 rounded-xl bg-muted/30 border border-border hover:border-primary/30 transition-colors group">
              ...
              <section>
                ...
                <div className="space-y-4">
                  {data.useCases.map((useCase, index) => (
                    <div key={index} className="flex items-start gap-4 p-5 rounded-xl bg-muted/30 border border-border hover:border-primary/30 transition-colors">
                    ...
                    {data.steps.map((step, index) => (
                      <div key={index} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                        ...
                        <div className="w-[calc(100%-3rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border border-border bg-card shadow-sm hover:border-primary/20 transition-colors">
                          ...
                          <div className="text-muted-foreground text-xs font-medium">{step.description}</div>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="card-glass p-8 border-primary/20">
                ...
                <ul className="space-y-4">
                  <li className="flex items-start gap-3">
                    ...
                    <span className="text-muted-foreground">Enterprise-grade reliability and 99.9% uptime validation.</span>
                  </li>
                  <li className="flex items-start gap-3">
                     <CheckCircle className="w-5 h-5 text-primary shrink-0 mt-1" />
                    <span className="text-muted-foreground">Seamless integration with existing fleet management workflows.</span>
                  </li>
                  <li className="flex items-start gap-3">
                     <CheckCircle className="w-5 h-5 text-primary shrink-0 mt-1" />
                    <span className="text-muted-foreground">Dedicated 24/7 technical support and rapid incident response.</span>
                  </li>
                </ul>
              </div>
            </div>
          ...
          <div className="rounded-3xl bg-gradient-primary p-12 text-center text-white relative overflow-hidden">
            ...
            <p className="text-xl text-white/80 mb-10 max-w-2xl mx-auto relative z-10">
              Transform your fleet operations with our industry-leading GPS tracking hardware and intuitive software platform.
            </p>
