...
import { gpsSolutions, SolutionData as GPSSolutionData } from "@/sections/gps/data/gpsData";
import { fastagSolutions, SolutionData as FastagSolutionData } from "@/sections/fastag/data/fastagData";

export default function FastagSection() {
  const [selectedSolution, setSelectedSolution] = useState<
    (typeof solutions)[0] | null
  >(null);
  const [selectedSolutionData, setSelectedSolutionData] =
    useState<GPSSolutionData | FastagSolutionData | null>(null);
  ...
  const handleLearnMore = (solution: (typeof solutions)[0]) => {
    ...
    if (solution.slug === "gps-tracking-solutions") {
      setSelectedSolutionData(gpsSolutions["gps-tracking-solutions"]);
    } else if (solution.slug === "fastag") {
      setSelectedSolutionData(fastagSolutions["fastag"]);
    } else {
